...
class RedisConnect(object):
    ...
    def connect(self):
        ...
        """  # noqa
        try:
            redis.StrictRedis(host=self.host, port=self.port,
                              password=self.password).ping()
        except redis.ConnectionError:
            raise RedisNoConnException(
                'Failed to create connection to redis',
                (self.host, self.port)
            )
...
class SimpleCache(object):
    def __init__(self,
        ...
        # Should we hash keys? There is a very small risk of collision invloved.   # noqa
    ...
    def get(self, key):
        key = to_unicode(key)
        # No need to validate membership,
        # which is an O(1) operation, but seems we can do without.
        if key:
    ...
    def decorator(function):
        ...
        def func(*args, **kwargs):
            ...
            try:
                ...
                logging.exception('Unknown redis-simple-cache error. Please check your Redis free space.')  # noqa
